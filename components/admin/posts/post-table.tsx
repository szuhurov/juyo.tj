...
export function PostTable({ rows }: { rows: AdminPostRow[] }) {
  ...
  if (rows.length === 0) {
    return <p className="py-16 text-center text-sm font-semibold text-zinc-400">Ягон эълон ёфт нашуд</p>;
  }
      ...
      <div className="divide-y divide-zinc-50 p-1.5 space-y-1">
        {rows.map((post) => {
          ...
          return (
            ...
            >
              <span className="text-zinc-400 font-semibold group-hover:text-blue-100">#{post.id.slice(-4)}</span>
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="font-semibold text-zinc-800 dark:text-zinc-100 group-hover:text-white truncate">{post.title}</p>
