...
export function UserSavedItems({ savedItems }: { savedItems: AdminUserDetail["savedItems"] }) {
  ...
  if (rows.length === 0) {
    return <p className="py-16 text-center text-sm font-semibold text-zinc-400">Ягон эълони захирашуда нест</p>;
  }
      ...
      <div className="divide-y divide-zinc-50 p-1.5 space-y-1">
        {rows.map((saved) => {
          ...
          return (
            ...
            >
              <span className="text-zinc-400 font-semibold group-hover:text-blue-100">#{item.id.slice(-4)}</span>

              <div className="flex items-center gap-3 min-w-0">
                <div className="w-10 h-10 rounded-md bg-zinc-100 dark:bg-zinc-800 overflow-hidden shrink-0 flex items-center justify-center">
                  {thumb ? (
                ...
                <div className="min-w-0">
                  <p className="font-semibold text-zinc-800 dark:text-zinc-100 group-hover:text-white truncate">{item.title}</p>
